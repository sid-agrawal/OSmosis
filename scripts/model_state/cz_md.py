...
# ./cz_md.py -f cz_md.csv &&  cp  cz_md.csv ~/neo4j/import/cz_md.csv && python import_csv.py --file cz_md.csv   
parser = argparse.ArgumentParser("cz_md")
...
if __name__ == "__main__":
    ...
    # Make Proc1-MD_1
        # PD
    pdd_id1 = model.add_pd_node("MD1_P1", 0x2)
    ...
    model.add_createdby_edge(kernel_id, ResourceType.PVA, crs_id1)
    
    crs_id2 = model.add_resource_space_node(ResourceType.PVA, "Key2")
    model.add_createdby_edge(kernel_id, ResourceType.PVA, crs_id2)

        # Add a CVA Resources
    cres_id1 = model.add_resource_node(ResourceType.PVA, crs_id1, "Key1")
    model.add_hold_edge(Permission.R, pdd_id1, ResourceType.PVA, crs_id1, cres_id1, [kernel_id])
    
    cres_id2 = model.add_resource_node(ResourceType.PVA, crs_id2, "Key2")
    model.add_hold_edge(Permission.R, pdd_id1, ResourceType.PVA, crs_id2, cres_id2, [kernel_id])
    ...
    # Make Proc1-MD_1
        # PD
    pdd_id2 = model.add_pd_node("MD1_P2", 0x3)
    model.add_hold_edge(Permission.R, pdd_id2, ResourceType.PVA, crs_id2, cres_id2, [kernel_id])
