...
void wordle_server_send() {
    ...
    for (int i = 0; i < WORD_LENGTH; i++) {
        sel4cp_mr_set(i, table[curr_row][i].ch);
    }
    sel4cp_ppcall(WORDLE_CHANNEL, sel4cp_msginfo_new(0, WORD_LENGTH));
    ...
    for (int i = 0; i < WORD_LENGTH; i++) {
        table[curr_row][i].state = sel4cp_mr_get(i);
    }
}
